...
class ScriptTask(GameUi, ReplaceShikigami, KekkaiUtilizeAssets):
    ...
    def run_utilize(self, friend: SelectFriendList = SelectFriendList.SAME_SERVER,
        ...
        # BOTH 模式保留本地已有的双区比较逻辑；怠惰模式下按两个分区
        # 依次尝试，仍遵循怠惰模式的“先五星、再回退四星”策略。
        if friend == SelectFriendList.BOTH and not self.utilize_lazy_mode_active:
            ...
            if friend == SelectFriendList.BOTH:
            ...
            elif self.utilize_lazy_mode_active:
                # 怠惰模式只处理配置指定的优先分组。
                friend_groups = (friend,)
            else:
                fallback_friend = (
                    SelectFriendList.DIFFERENT_SERVER
                    if friend == SelectFriendList.SAME_SERVER
                ...
                friend_groups = (friend, fallback_friend)
    ...
    def _run_utilize_single(self, friend: SelectFriendList) -> bool:
        ...
        return self._select_optimal_resource_card(friend) is True
    ...
    def _select_best_server(self, records: dict, prefer: SelectFriendList) -> dict | None:
        ...
        for server, rec in records.items():
            taiko_ok = rec['jade'] > 0 and (
                thresholds['太鼓'] <= 0 or rec['jade'] >= thresholds['太鼓']
            )
            fish_ok = rec['ap'] > 0 and (
                thresholds['斗鱼'] <= 0 or rec['ap'] >= thresholds['斗鱼']
            )
            if rule == UtilizeRule.TAIKO:
                ...
                jade_idx = MAX_INDEX
            elif rule == UtilizeRule.DEFAULT:
                if not taiko_ok and not fish_ok:
                    continue
                ap_idx = get_idx('斗鱼', rec['ap']) if fish_ok else MAX_INDEX
                jade_idx = get_idx('太鼓', rec['jade']) if taiko_ok else MAX_INDEX
            else:
                logger.error('Unknown utilize rule')
                raise ValueError('Unknown utilize rule')
            if ap_idx == MAX_INDEX and jade_idx == MAX_INDEX:
                logger.info(f'[{server.value}] 可用卡均低于标准档位，按实际阈值继续比较')
            # 同档位优先太鼓
            if taiko_ok and jade_idx <= ap_idx:
    ...
    def _select_by_record(
        self,
        best_card_type: str | None = None,
        best_card_num: int | None = None,
    ) -> bool:
        """
        基于已记录的最优值，在当前区按 _current_select_best 的确认模式选卡
        """
        if best_card_type is None or best_card_num is None:
            best = self._select_best_server(
                {
                    SelectFriendList.SAME_SERVER: {
                        'ap': self.ap_max_num,
                        'jade': self.jade_max_num,
                    }
                },
                prefer=SelectFriendList.SAME_SERVER,
            )
            if best is None:
                return False
            best_card_type = best['res_type']
            best_card_num = best['res_val']

        logger.info(f'⚖️ 按记录确认选卡: {best_card_type} @ {best_card_num}')
        return bool(
    ...
    def _current_select_best(
        ...
        ``selected_card`` 用于双区模式按记录值重新选卡，``explore_only``
        用于双区比较时完整扫描当前分组而不提前结束。
        """
        # ============== 配置常量 ==============#
        RESOURCE_CONFIG = {
            ...
            for target, _, area, _ in cards:
                ...
                if selected_card:  # 确认选择模式
                    ...
                    if (card_type == best_card_type) and (card_value >= best_card_num):
                        ...
                        return True
                elif not explore_only and self._is_strategy_maximum_reward(card_type, card_value):
                    logger.info(
                        f'✅ 当前策略最高收益已确认: '
